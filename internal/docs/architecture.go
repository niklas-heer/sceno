...
func buildArchitectureDoc() ArchitectureDoc {
	return ArchitectureDoc{
		...
		Pipeline: []string{
			...
			"validate / advise / describe / preview / render — consumers of pipeline.Result",
		},
		...
		Consumers: map[string]string{
			...
			"sceno advise":   "Per-slide EngineReport geometry and spacing, plus merged score/findings; engine_slide_index identifies the representative lowest-score slide",
			...
			"sceno preview":  "validate.EvaluateSource evaluates current bytes once per revision; sanitized SVG and source-linked geometry share pipeline.Result; internal/repair verifies proposed edits before explicit Apply",
		},
		...
		Principles: []string{
			...
			"Local preview binds loopback with session authentication, watches atomic editor saves, checks revisions before writes, and preserves invalid source for repair without rendering it",
			"Repair preview re-evaluates exact offered source edits and reports measured movement; Apply rechecks the current revision and evidence; Undo is limited to changes from the current session",
			"Preview always uses polished SVG; invalid source retains a marked last-valid canvas with inspection disabled; exports are checked against the visible source revision",
			"Polished PNG embeds text as Inter glyph paths at their original SVG paint positions, normalizes scalar transforms and rounded corners for rasterization, and returns errors instead of silently switching to a different renderer",
			"CLI --style sketch uses a separate unframed SVG/PNG renderer with limited shape/icon/text parity; PDF, HTML, and framed slide outputs stay polished",
			...
			"Spacing: scene.AnalyzeSpacing consumes computed stack bounds and content boxes; reports bounds-based node clearance, signed content/parent padding, canvas margins, and explicit pair truncation in pixels",
			"Deck feedback: slide indices are 1-based; merged findings retain slide_index, and advise.slides[] preserves every slide's complete engine for AI review",
