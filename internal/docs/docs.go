...
func buildVisualDoc() VisualDoc {
	return VisualDoc{
		...
		Metrics: map[string]string{
			...
			"measured_spacing":             "slides[n].engine.spacing: px, axis-aligned node bounds; signed padding, explicit truncation, and overlap exemptions",
			"minimum_node_clearance":       "half the diagram gap, measured on each axis; at least one axis must meet it",
		...
		ReviewLoop: []string{
			...
			"inspect slides[n].engine.spacing for actual gaps and padding; negative insets mean overflow; inspect truncated before treating pair feedback as complete",
...
func writeValidationMarkdown(w io.Writer) error {
	...
	b.WriteString("- Edge render checks (same math as export): `arrow_detached`, `arrow_cluster`, `edge_label_chrome_overlap`, `edge_label_overlap`, `edge_label_off_axis`\n")
